            ...
            <div class="box__avisos-body">
                <div class="list-group">
                    ...
                    while ($user_data = mysqli_fetch_assoc($result)) {
                        ...
                        $visivel = $user_data['visivel'];
                        $titulo = $user_data['titulo'];
                        $descricao = $user_data['descricao'];
                        $id_comunidade = $user_data['id_comunidade'];

                        // Os avisos serão filtrados pelo visivel (0 - somente a comunidade verá, 1 - todos da paróquia) e pelo id da comunidade (somente verá se os avisos forem da comunidade do usuário, em caso de visivel == 0)

                        // visivel == 1 ou (visivel == 0 e comunidade de quem inseriu == comunidade de quem visualizou) {visto} 
                        if ($codPerfil == 2 || $visivel == 1 || ($visivel == 0 && $id_comunidade == $_SESSION['id_comunidade'])) {
                            // echo "Inserido por: " . $id_comunidade . " # Visualizado por: " . $_SESSION['id_comunidade'] . " # visivel: " . $visivel;

                            echo "<a id='item-aviso-$id_aviso' class='list-group-item' aria-current='true'>";
                            ...
                            <small class='small-visivel";
                                if ($visivel == 0) {
                                    echo " comunidade'>Comunidade</small>";
                                } else {
                                    echo " paroquia'>Paróquia</small>";
                                }
                                echo "<button class='btn btn-sm' data-bs-toggle='modal'                 data-bs-target='#modal-avisos' onclick='setarModalAvisoUpdate(\"$titulo\", \"$descricao\", $visivel, $id_aviso, $id_comunidade)'>
                                    <svg xmlns='http://www.w3.org/2000/svg' fill='blue' class='bi bi-pencil-fill' viewBox='0 0 16 16'>
                                        <path d='M12.854.146a.5.5 0 0 0-.707 0L10.5 1.793 14.207 5.5l1.647-1.646a.5.5 0 0 0 0-.708l-3-3zm.646 6.061L9.793 2.5 3.293 9H3.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.207l6.5-6.5zm-7.468 7.468A.5.5 0 0 1 6 13.5V13h-.5a.5.5 0 0 1-.5-.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.5-.5V10h-.5a.499.499 0 0 1-.175-.032l-.179.178a.5.5 0 0 0-.11.168l-2 5a.5.5 0 0 0 .65.65l5-2a.5.5 0 0 0 .168-.11l.178-.178z'/>
                                    </svg>
                                </button>
...
function setarModalAvisoUpdate(titulo, descricao, visivel, id_aviso, id_comunidade) {
